...
def compute_reward_star(num_agents, old_positions, agent_positions, evacuated_agents, deactivated_agents, goal_area, 
                   grid=None, walls=None, dynamic_obstacles=None):
    """
    ...
    * pas compute le full path si pas besoin
    ...
    with open('reward_log.txt', 'a') as log_file:
        ...
        log_file.write(f"Number of walls: {len(walls) if walls is not None else 'None'}\n")
    ...
    for i, (old_pos, new_pos) in enumerate(zip(old_positions, agent_positions)):
        ...
        if i in evacuated_agents:
            ...
            if grid is not None:
                ...
                closest_goal = min(goal_area, key=lambda g: abs(new_pos[0] - g[0]) + abs(new_pos[1] - g[1]))
                ...
                if new_path_length < old_path_length:
                    ...
                    progress = (old_path_length - new_path_length) / max(1, old_path_length)
                    rewards[i] += 10.0 * progress
                    
                    # Log the path improvement
                    with open('reward_log.txt', 'a') as log_file:
                        log_file.write(f"Agent {i+1}: PROGRESS - Old path: {old_path_length}, New path: {new_path_length}, Reward: +{10.0 * progress:.2f}\n")
                elif new_path_length > old_path_length:
                    # Penalty for moving away from the goal 
                    regress = (new_path_length - old_path_length) / max(1, old_path_length)
                    rewards[i] -= 5.0 * regress
                    # Log the path regression
                    with open('reward_log.txt', 'a') as log_file:
                        log_file.write(f"Agent {i+1}: REGRESS - Old path: {old_path_length}, New path: {new_path_length}, Penalty: -{5.0 * regress:.2f}\n")
